...
//Печатает приглашение, читает строку с клавиатуры и очищает её от перевода строки.
...
//Читает ответ сервера через recv и печатает его.
...
//Из полного пути достаёт только имя файла.
...
static void run_file_client(int sock) {
    ...
    if (read_line_stdin("Enter file path: ", path, sizeof(path)) != 0) {
